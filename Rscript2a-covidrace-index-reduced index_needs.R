...
#raw data used in 'Rscript1-covidrace-clean.R' are not shared
covid.index <- read.table('data-covidrace-v2-cleaned.csv', header=TRUE, sep=",", dec=".", stringsAsFactors = TRUE)

#index_Needs: branching logic did not work and several answer combinations are implausible
#reduce sample size by cutting individuals with implausible answers
#in those cases, needs met was code by the needs met questions only, disregarding the income
...
#save data
write.csv(covid.index, "data-covidrace-v3-indexed-reduced index_needs.csv", quote=FALSE, row.names = FALSE)
