...
covid <- read.table('data-covidrace-v3-indexed-modified index_needs.csv',header=TRUE, stringsAsFactors = TRUE, sep=",", dec=".")
covid.modified <- covid[!is.na(covid$Index_Needs),]
rm(covid)

#Food Changes
#create dataset
foodchanges <- dplyr::select(covid.modified, US_Race, MinorityEthnic_Race_Status, MinorityEthnic_Race_Status_Binary,
...
write.csv(foodchanges, "data-covidrace-foodchanges-WIDE-modified index_needs.csv", quote=FALSE, row.names = FALSE)
...
#write data
write.csv(foodchanges.long, "data-covidrace-foodchanges-LONG-modified index_needs.csv", quote=FALSE, row.names = FALSE)
rm(foodchanges.long)

#Household
#create dataset
household.modified <- dplyr::select(covid.modified, US_Race, MinorityEthnic_Race_Status, Gender, Index_SES, Index_Needs,
                    Index_Household, Index_Environment,
                    SmellTasteLoss_Cause_VirusSpecific, ZipCode)
#write data
write.csv(household.modified, "data-covidrace-household-modified index_needs.csv", quote=FALSE, row.names = FALSE)
rm(household.modified)

#Dry Mouth
#create dataset
drymouth.modified <- dplyr::select(covid.modified,  US_Race, MinorityEthnic_Race_Status,
...
drymouth.modified <- rename(drymouth.modified, Index_DryMouth = Symptom_DryMouth)

#write data
write.csv(drymouth.modified, "data-covidrace-drymouth-modified index_needs.csv", quote=FALSE, row.names = FALSE)
rm(drymouth.modified)

#Relative SES
#create dataset
relativeSES <- dplyr::select(covid.modified, US_Race, MinorityEthnic_Race_Status, Gender, Sexual_Orientation,
...
#write data
write.csv(relativeSES, "data-covidrace-relativeSES-modified index_needs.csv", quote=FALSE, row.names = FALSE)
